...
> ⚠️ The body is capped at 5 KB — fine for a health check or a JSON error stub, wrong for anything meant to stand in for a real payload (see Limits and Sources).
...
**Four policy kinds sit between a request arriving and a backend ever seeing it** — each is a member of the deployment specification's `requestPolicies` object (*The deployment specification*, above), and each answers a different question:
...
- Parameter names allow letters, digits, and underscores only (see Limits and Sources).
...
`orders-service` needs none of the internet-gateway piece: it's an internal Load Balancer address inside the same VCN, reached without ever touching the public internet.
...
One `routingBackends` entry can be marked `isDefault` — the catch-all when no other key matches, so an unrecognized subdomain doesn't simply fail to route anywhere.
...
| A stock response body is capped at 5 KB | Stock responses are a routing tool, not a mocking tool — a stubbed backend contract outgrows the cap as soon as it returns a realistic object | Jul 2026, [docs](https://docs.oracle.com/en-us/iaas/Content/APIGateway/Tasks/apigatewayaddingstockresponses.htm) |
