...
| Blue-green on OKE needs 2 pre-created namespaces + an NGINX ingress | Check for the ingress controller before choosing a release strategy — installing one is a cluster-level change, not a pipeline setting | Jul 2026, [docs](https://docs.oracle.com/en-us/iaas/Content/devops/using/bgoke_deploy.htm) |
...
| Native repos trigger on push only; PR events need an external connection | A PR-gated workflow rules out OCI-native code repositories — pick GitHub/GitLab/Bitbucket at project setup, not after | Jul 2026, [docs](https://docs.oracle.com/en-us/iaas/Content/devops/using/trigger_build.htm) |
| No native cron — triggers fire on source events only | Budget for an external scheduler you host and secure yourself — there is no schedule field on the trigger to fill in | Jul 2026, [docs](https://docs.oracle.com/en-us/iaas/Content/devops/using/trigger_build.htm) |
| Approval stage times out after a default 7-day window | Route approval notifications to a channel someone actually watches (the ONS topic in *Project structure*, above) — the gate expires silently, it doesn't escalate | Jul 2026, [docs](https://docs.oracle.com/en-us/iaas/Content/devops/using/approval_stage.htm) |
